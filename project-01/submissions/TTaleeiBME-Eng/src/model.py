"""
Helper module – Maternal Health Risk Stratification (v4)
- 3-class classification (Low / Mid / High)
- sklearn Pipeline (no leakage)
- class_weight='balanced' to improve High-risk Recall
- Explicit handling of label noise (identical features, different RiskLevel)
"""
...
def load_and_clean(path: str) -> pd.DataFrame:
    ...
    1. Remove implausible HeartRate (< 40)
    2. Remove true label conflicts (identical features but different RiskLevel)
    3. Remove exact duplicates
    """
    df = pd.read_csv(path)

    # 1. Implausible values
    df = df[df["HeartRate"] >= 40].copy()

    # 2. True label noise: same features, different target
    df["feature_tuple"] = df[FEATURES].apply(tuple, axis=1)
    nunique = df.groupby("feature_tuple")[TARGET].transform("nunique")
    df = df[nunique == 1].copy()
    df = df.drop(columns=["feature_tuple"])
...
def tune_tree(X_train, y_train, random_state: int = 42):
    """
